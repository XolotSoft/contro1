...
class User extends Main
{
    ...
    public function setUsername($value)
    {
        if (Val::requerido($value)) {
            ...
            $this->Util()->setError(0, 'error', 'Nombre de usuario :: Campo requerido');
        }
    }
    ...
    public function doLogin()
    {
        ...
        if ($row) {
            if ($row['activo'] == "1") {
                ...
                $card['isLogged'] = true;
                $_SESSION['User'] = $card;
                $_SESSION['Empresa'] = "controlobra_".$row['empresas_id'];
                return true;
            } else {
                $this->Util()->setError(0, 'error', 'El usuario esta inactivo.');
                $this->Util()->PrintErrors();
                return false;
            }
        } else {
            $this->Util()->setError(0, 'error', 'Tu usuario o contraseña es incorrecta.');
            $this->Util()->PrintErrors();
            return false;
        }
        // } else {
